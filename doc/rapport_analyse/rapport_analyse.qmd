...
La formule prévue sera de la forme :
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| `/create_user` | POST | `username`, `password`, `email` | `User` | connecté | Crée un compte |
| `/login` | POST | `username`, `password` | jeton d'accès | connecté | Authentifie et renvoie un jeton |
| `/users` | GET | - | `liste[User]` | admin | Liste les comptes |
| `/users/{id_user}` | PATCH | `username` et/ou `password` | `User` | admin | Modifie un compte |
| `/users/{id_user}` | DELETE | - | - | admin | Supprime un compte |

#### Stations

| Chemin | Méthode | Entrée | Sortie (succès) | Habilitation | Description |
| :---- | :---- | :---- | :---- | :---- | :---- |
| `/stations` | GET | - | `liste[Station]` | connecté | Liste les stations |
| `/stations/{id_station}` | GET | - | `Station` | connecté | État actuel d'une station + indicateurs statistiques + score |
| `/stations /{id_station} /history` | GET | période (24h/7j/30j) | `liste[Releve]` | connecté | Évolution de la disponibilité |
| `/suggestions` | GET | position géographique | `liste[Station]` | connecté | Stations recommandées en fonction des critères |

#### Gestion des stations favorites

| Chemin | Méthode | Entrée | Sortie (succès) | Habilitation | Description |
| :---- | :---- | :---- | :---- | :---- | :---- |
| `/favorites` | GET | - | `liste[Station]` | connecté | Favoris de l'utilisateur courant |
| `/favorites/{station_id}` | PUT | - | - | connecté | Ajoute une station aux favoris |
| `/favorites/{station_id}` | DELETE | - | - | connecté | Retire une station des favoris |



### Diagramme de séquence d'une fonctionnalité  

On présente ci-dessous le diagramme de séquence de la fonctionnalité "ajouter une station favorite", afin de visualiser le lien entre les différentes couches. Ce diagramme traite du cas où la station n'est pas déjà en favori.
