...
Le cahier des charges prévoit un minimum de fonctionnalités attendues. La @fig-cas_utilisation résume les actions que pourront effectuer les utilisateurs. Ces actions sont détaillées ci-dessous avec, pour chacune, la solution technique envisagée.  

![Diagramme de cas d'utilisation](img/diag_cas_utilisation.png){#fig-cas_utilisation}   

**Collecter des données via une API externe**  

La collecte des données concernant la disponibilité et l'état des stations de vélos se fera via une API externe d'une ville. Ce genre d'API respecte le standard GBFS, c'est-à-dire qu'elles transmettent plusieurs fichiers JSON formatés permettant de connaître les stations et leurs disponibilités. Les fichiers qui nous intéresseront seront les fichiers ```station_information.json```[^1] (donnant des informations sur les stations) et ```station_status.json```[^2] (donnant des informations sur les disponibilités). Ces fichiers, qui seront collectés à intervalles de temps réguliers, permettront de nourrir une base de données. Cette dernière sera hébergée sur un service PostgreSQL et contiendra notamment une table pour les stations, et une pour les relevés (voir la @fig-modele_donnees pour plus de détails).  

[^1]: disponible ici : [https://eu.ftp.opendatasoft.com/star/gbfs/station_information.json](https://eu.ftp.opendatasoft.com/star/gbfs/station_information.json). Ce fichier possède les variables d'intérêts suivantes : l'identifiant de la station ```station_id```, son nom ```name```, ses coordonnées géographiques ```lon``` et ```lat```, ainsi que son nombre d'emplacements ```capacity```.  

[^2]: disponible ici : [https://eu.ftp.opendatasoft.com/star/gbfs/station_status.json](https://eu.ftp.opendatasoft.com/star/gbfs/station_status.json). Ce fichier possède les variables d'intérêts suivantes : l'identifiant de la station ```station_id```, le nombre de vélos disponibles ```num_bikes_available```, le nombre d'emplacements libres ```num_docks_available``` et le fait qu'elle soit en service ou non ```is_installed```.  
...
La @fig-activite présente un diagramme d'activité résumant les étapes de navigation dans l'API.

![Diagramme d'activité](img/diag_activite.png){#fig-activite width="98%"}   

{{< pagebreak >}}

**Gestion de compte**  

La partie haute de ce diagramme correspond à la phase de connexion de l'utilisateur. L'utilisateur peut ainsi créer un compte s'il n'en a pas ou se connecter à son compte. Seul un administrateur dispose, après connexion à son compte, de la possibilité d'influer sur les comptes des utilisateurs «réguliers».

**Fonctionnalité principales**  

L'accès aux fonctionnalités de l'API se situe dans la partie basse du diagramme. L'ensemble des options (recommandations, liste, favoris et recherches) mènent à la consultation des données liées à une seule station. L'utilisateur a ensuite le choix de quitter l'API ou de poursuivre la navigation en retournant à l'étape du choix d'action.  
...
L'API sera constituée de plusieurs couches afin de respecter le principe de séparations des responsabilités. L'architecture logicielle envisagée est présentée dans la @fig-archi-log.

![Diagramme d'architecture logicielle](img/diag_archi_log.png){#fig-archi-log}  
...
Afin que le modèle de données soit cohérent et pérenne, nous veillerons à respecter les trois premières formes normales. Le modèle de données retenu est résumé en @fig-modele_donnees. Les raisons ayant amené à ce choix sont expliquées ci-dessous.  
...
![Diagramme de classes lié aux stations](img/diag_classes_station.png){#fig-classes-station width="97%"}   
...
À ce stade, nous avons une vue d'ensemble du fonctionnement de l'application. Nous pouvons résumer ce qui a été vu précédemment grâce à un diagramme de séquence. Pour l'exemple, la @fig-sequence présente le diagramme de séquence de la fonctionnalité «ajouter une station favorite» lorsque la station n'est pas déjà en favori.  
...
Le calendrier prévu est donnée par le diagramme de Gantt en @fig-diag_gantt.  

![Diagramme de Gantt](img/diag_gantt.png){#fig-diag_gantt width="92%"}
